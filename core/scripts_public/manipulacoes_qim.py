...
def comparar_excel(arquivo_antigo, arquivo_novo, manter_duplicados=False, pa = None):
    ...
    try:
        ...
        # Se o arquivo não existir, retornar o arquivo_novo como está
        print(f"Arquivo {arquivo_antigo} não encontrado. Criando novo arquivo.")
        return arquivo_novo

    if not manter_duplicados:
        # Encontrar o último valor de 'id_pa' no arquivo antigo
        ultimo_id_pa = copy['id_pa'].max() if not copy.empty and 'id_pa' in copy.columns else 0
        ...
        combined = pd.concat([copy, arquivo_novo], ignore_index=False)
