...
def comparar_excel():
    ...
    copy_empresas = pd.read_excel(
        os.path.abspath(os.path.join(DWPII_COPY, "informacoes_empresas.xlsx"))
    )
    up_projetos = pd.read_excel(os.path.abspath(os.path.join(DWPII_UP, "portfolio.xlsx")))
    up_empresas = pd.read_excel(
        os.path.abspath(os.path.join(DWPII_UP, "informacoes_empresas.xlsx"))
    )
