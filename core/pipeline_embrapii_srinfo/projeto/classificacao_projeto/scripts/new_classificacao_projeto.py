...
def new_classificacao_projeto():
    ...
    path_projetos_empresas = os.path.join(
        ROOT, "projeto", "classificacao_projeto", "step_1_data_raw", "projetos_empresas.xlsx"
    )
    path_projetos_contratados = os.path.join(
        ROOT, "projeto", "classificacao_projeto", "step_1_data_raw", "raw_projetos_contratados.xlsx"
    )
    path_informacoes_empresas = os.path.join(
        ROOT, "projeto", "classificacao_projeto", "step_1_data_raw", "informacoes_empresas.xlsx"
    )
    path_cnae = os.path.join(
        ROOT, "projeto", "classificacao_projeto", "step_1_data_raw", "raw_cnae_divisao.xlsx"
    )
    path_ues = os.path.join(
        ROOT, "projeto", "classificacao_projeto", "step_1_data_raw", "raw_competencias_ues.xlsx"
    )
    path_destino = os.path.join(
        ...
        "step_2_stage_area",
        "new_classificacao_projeto.xlsx",
    )
    ...
    df_informacoes_empresas = df_informacoes_empresas[
        ["cnpj", "empresa", "faixa_faturamento", "cnae_subclasse", "porte"]
    ]
    ...
    df_aggregated = (
        df_merged.groupby("codigo_projeto")
        .agg(
            {
    ...
    df_aggregated["Data do contrato"] = pd.to_datetime(
        df_aggregated["Data do contrato"], format="%d/%m/%Y"
    )
