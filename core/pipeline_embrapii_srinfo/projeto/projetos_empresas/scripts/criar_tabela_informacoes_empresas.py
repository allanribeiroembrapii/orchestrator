...
def criar_tabela_informacoes_empresas():
    ...
    df_atual = pd.read_excel(
        os.path.join(
            ROOT, "projeto", "projetos_empresas", "step_1_data_raw", "raw_informacoes_empresas.xlsx"
        )
    )  # informacoes_empresas atual
    df_atual["novo"] = "Não"
    df_atual.to_excel(
        os.path.join(
            ROOT, "projeto", "projetos_empresas", "step_1_data_raw", "raw_informacoes_empresas.xlsx"
    ...
    info_empresas = pd.read_excel(
        os.path.join(ROOT, "projeto", "projetos_empresas", "step_1_data_raw", "raw_empresas.xlsx")
    )  # info_empresas novo
    empresas_contratantes = pd.read_excel(
        os.path.join(
            ROOT,
            "analises_relatorios",
            "empresas_contratantes",
            "step_1_data_raw",
            "raw_relatorio_empresas_contratantes_1.xlsx",
        )
    )  # empresas_contratantes
    ...
    novos_registros = pd.merge(
        novos_registros, empresas_contratantes, how="left", left_on="cnpj", right_on="CNPJ"
    )
    novos_registros = pd.merge(novos_registros, info_empresas, how="left", on="cnpj")
    novos_registros["porte"] = novos_registros["Faixa de faturamento declarada"].apply(
        determinar_porte
    )
    ...
    # Renomear as colunas
    novos_registros = novos_registros.rename(
        columns={
            "Empresa": "empresa",
            "endereco_uf": "uf",
    ...
    caminho_temp = os.path.join(
        ROOT, "projeto", "projetos_empresas", "temp_informacoes_empresas.xlsx"
    )
