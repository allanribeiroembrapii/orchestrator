...
def ajustar_equipe_ue(arquivo_destino=arquivo_destino):
    ...
    equipe_ue["ano_entrada"] = pd.to_datetime(
        equipe_ue["data_entrada"], dayfirst=True, errors="coerce"
    ).dt.year
    ...
    equipe_ue = equipe_ue.groupby(["ano_entrada", "cpf"], group_keys=False).apply(
        obter_maior_titulacao
    )
