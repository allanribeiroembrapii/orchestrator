...
def criar_tabela_ue_linhas_atuacao():
    ...
    # Renomear as colunas
    df_renomeado = df_selecionado.rename(
        columns={"Unidade EMBRAPII": "unidade_embrapii", "Linhas de Atuação": "linha_atuacao"}
    )

    # Quebrar os valores da coluna "linha_atuacao" por ";" e transformar em linhas separadas
    df_explodido = df_renomeado.assign(
        linha_atuacao=df_renomeado["linha_atuacao"].str.split(";")
    ).explode("linha_atuacao")
