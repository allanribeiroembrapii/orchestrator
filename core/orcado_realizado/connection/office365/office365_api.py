...
class SharePoint:
    ...
    def download_file(
        ...
        conn = self._auth(sharepoint_site)
        file_url = (
            f"/sites/{sharepoint_site_name}/{sharepoint_doc}/{folder_name}/{file_name}"
        )
    ...
    def download_latest_file(
        self, sharepoint_site, sharepoint_site_name, sharepoint_doc, folder_name
    ):
        date_format = "%Y-%m-%dT%H:%M:%SZ"
        files_list = self._get_files_list(
            sharepoint_site, sharepoint_doc, folder_name)
        file_dict = {}
        for file in files_list:
            dt_obj = datetime.datetime.strptime(
                file.time_last_modified, date_format)
            file_dict[file.name] = dt_obj
        # sort dict object to get the latest file
        file_dict_sorted = {
            key: value
            for key, value in sorted(
                file_dict.items(), key=lambda item: item[1], reverse=True
            )
        }
    ...
    def upload_file(
        ...
        conn = self._auth(sharepoint_site)
        target_folder_url = (
            f"/sites/{sharepoint_site_name}/{sharepoint_doc}/{folder_name}"
        )
        target_folder = conn.web.get_folder_by_server_relative_path(
            target_folder_url)
        response = target_folder.upload_file(
            file_name, content).execute_query()
        return response
    ...
    def get_file_properties_from_folder(
        self, sharepoint_site, sharepoint_doc, folder_name
    ):
        files_list = self._get_files_list(
            sharepoint_site, sharepoint_doc, folder_name)
        properties_list = []
    ...
    def file_exists(self, folder_name, file_name):
        """
        Verifica se um arquivo existe no SharePoint
        """
        try:
            ...
            print(f"Erro ao verificar existência do arquivo: {str(e)}")
            return False
